...
int main(int, char**) {
	...
	while (true) {
		...
		while (running)
		{
			...
			String telemetryData = Telemetry.getData();

			if (telemetryData == "SW_ON") {

				Telemetry.sendData(string "SW_IS_ON");
			...
			else if (telemetryData == "SW_OFF") {
				Telemetry.sendData(string "SW_IS_OFF");
				running = false;
				return running;
			}
